...
#' @export
#'
get_drm <- function(data, activity_colname="activity", ...){
  ...
  return(drc::drm(get({{ activity_colname }})~conc_logM,
...
get_EC_logM <- function(data, trt, tgt, response_level=50,
  ...
  data <- filter_trt_tgt(data, trt, tgt, trt_colname, tgt_colname)
  drm <- get_drm(data, activity_colname=activity_colname)
