...
#'
#' * "response" (default) or other response column name (see `response_col`)
#' * log_dose: dose of treatment in log10 units
#' @param response_col Name of the column containing response values.
#'   Default is "response". Typical alternatives might include e.g.
#'   "percent_inhibition", "viability", "growth", "activity".
#' @param ... Additional arguments to be passed to [drda::drda()], e.g. bounds.
#' @return A dose-response model object of class `drda`.
#'
get_drda_helper <- function(data, response_col="response", ...){
  assertthat::assert_that(assertthat::has_name(data, response_col),
                          msg = glue::glue("column '{response_col}' not found"))
  assertthat::assert_that(all(data[[response_col]] != -Inf),
                          msg = "data contains -Inf values")
  # 4-param logistic model on pre-log-transformed data
  return(drda::drda(get({{ response_col }})~log_dose, data = data, ...))
}
...
#'
#' If the height of the model is positive (if response increases at higher
...
#' response units like percent inhibition that should not realistically exceed
  ...
  tryCatch({
    ...
    if(unbounded_delta < 0){
      ...
      dec_coeffs <- stats::coefficients(
        get_drda_helper(data, response_col,
...
#' @export
#'
get_drda_rigid <- function(data, response_col="response"){
  # first get coefficients of a model with no bounds on parameters
  tryCatch({
    unbounded_coeffs <- stats::coefficients(get_drda_helper(data, response_col))
    unbounded_delta <- unbounded_coeffs["delta"] # height of curve
...
get_drda_fixed <- function(data, alpha=NA, delta=NA, eta=NA, phi=NA,
  ...
  if(all(is.na(params))) return(get_drda_helper(data, response_col))
  ...
  get_drda_helper(data, response_col, lower_bound = lower,
                  upper_bound = upper)
}
