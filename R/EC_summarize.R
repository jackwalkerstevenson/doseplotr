...
#' @param rigid Whether to set a rigid low-dose asymptote with
#'   `get_drda_rigid()`
#' @param bounded Whether to set bounds on the model asymptotes at all
...
#' @export
summarize_models <- function(data,
  ...
  data |> dplyr::group_by(.data$treatment, .data$target) |>
    dplyr::summarize(
      model =
        ...
        list(data |> filter_trt_tgt(.data$treatment, .data$target) |>
               get_drda(response_col, bounded = FALSE))
          } else{
        list(data |> filter_trt_tgt(.data$treatment, .data$target) |>
      ...
      min_log10_dose = min({data |> filter_trt_tgt(.data$treatment, .data$target)}$log_dose),
      max_log10_dose = max({data |> filter_trt_tgt(.data$treatment, .data$target)}$log_dose)
    )
}
